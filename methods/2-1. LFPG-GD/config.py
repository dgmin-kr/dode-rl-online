...
_DEFAULT_NETWORK_NAME = canonical_network_name(get_network_name("lfpg_gd"))
...
def _resolve_settings(network_name: str) -> tuple[str, dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    return (
        ...
        get_experiment_settings("lfpg_gd", canonical_name),
...
class Config:
    ...
    LFPG_GD_PARAMS: dict[str, Any] = dict(_EXPERIMENT_SETTINGS["lfpg_gd_params"])
    ...
    def configure_network(cls, network_name: str) -> None:
        resolved_network_name, common_settings, utils, dnl_settings = _resolve_settings(network_name)
        ...
        cls.ACTION_HIGH = resolve_action_high_setting(cls.NETWORK_NAME, common_settings, utils)
        ...
        cls.ALGORITHM = str(utils["algorithm"])
        cls.LFPG_GD_PARAMS = dict(utils["lfpg_gd_params"])
