...
from w_spsa import solve_w_spsa_step


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Weighted SPSA baseline over the shared test split."
    )
    ...
    parser.add_argument("--max-iterations", type=int, default=None, help="Optional override for local SPSA iterations.")
...
def run_one_trial(
    ...
    *,
    ...
    if not test_scenarios:
        raise ValueError("Test split is empty. Generate a scenario dataset before running W-SPSA.")

    params = dict(Config.W_SPSA_PARAMS)
    ...
    print(
        f"[W-SPSA] selected test scenarios={selected_test_scenario_ids}; "
    ...
    for scenario_sample in test_scenarios:
        ...
        scenario_run = run_step_locked_scenario(
            ...
            step_solver=lambda context, local_params=params, local_state=scenario_state: solve_w_spsa_step(
                context,
                params=local_params,
                state=local_state,
            ),
        ...
        print(
            f"[W-SPSA] scenario={scenario_sample.scenario_id} "
