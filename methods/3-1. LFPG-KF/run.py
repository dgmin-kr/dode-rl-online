...
from utils.kalman_filtering import solve_kf_step


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="LFPG-KF online baseline over the shared test split.")
    ...
    parser.add_argument("--seed", type=int, default=6000, help="Base random seed.")
...
def run_one_trial(
    ...
    *,
    ...
    if not test_scenarios:
        raise ValueError("Test split is empty. Generate a scenario dataset before running LFPG-KF.")

    params = dict(Config.KF_PARAMS)
    ...
    print(
        f"[{Config.ALGORITHM}] selected test scenarios={selected_test_scenario_ids}; "
    ...
    for scenario_sample in test_scenarios:
        ...
        scenario_state: dict[str, object] = {"previous_action": None}
        scenario_run = run_step_locked_scenario(
            config=Config,
            scenario_sample=scenario_sample,
            simulation_seed=simulation_seed,
            step_solver=lambda context, local_params=params, local_state=scenario_state: solve_kf_step(
                context,
                params=local_params,
                state=local_state,
                lfpg_enabled=bool(Config.LFPG_ENABLED),
            ),
            step_runtime_seconds=step_runtime_seconds,
        )
        scenario_runs.append(scenario_run)
        print(
            f"[{Config.ALGORITHM}] scenario={scenario_sample.scenario_id} "
    ...
    config_snapshot = {
        ...
        "seed": int(seed),
        "kf_params": params,
        "lfpg_guidance_enabled": bool(Config.LFPG_ENABLED),
